...
function comment_select_by_users($product_id)
{
    ...
    $sql = "select comments_product.*,users.name as name_person_comment from comments_product join users  on comments_product.user_id=users.id  WHERE product_id ='$product_id' ORDER BY created_at";
    return pdo_query($sql);
}
